...
struct IndexGeneratedResultCardList: View {
    let items: [IndexResultCardItem]
    var emptyText = IndexEmptyStateCopy.noResults

    var body: some View {
        IndexGeneratedResultCardListPresence(items: items, emptyText: emptyText)
    }
}
...
private struct IndexGeneratedResultCardListPresence: View {
    let items: [IndexResultCardItem]
    let emptyText: String

    @Environment(\.accessibilityReduceMotion) private var reduceMotion
    @State private var presentation: IndexResultPresenceState<[IndexResultCardItem]>
    @State private var resultOpacity: Double
    @State private var completionRequest: IndexResultPresenceCompletion?
    @State private var animatesItemUpdates = false

    init(items: [IndexResultCardItem], emptyText: String) {
        self.items = items
        self.emptyText = emptyText

        let initialValue = items.isEmpty ? nil : items
        _presentation = State(initialValue: IndexResultPresenceState(initialValue: initialValue))
        _resultOpacity = State(initialValue: initialValue == nil ? 0 : 1)
    }
    ...
    private var resultInteractionEnabled: Bool {
        !items.isEmpty && presentation.phase != .empty && presentation.phase != .exiting
    }

    var body: some View {
        visibleContent
            .task(id: updateID) {
                applyTarget(items.isEmpty ? nil : items, reduceMotion: reduceMotion)
            }
            .onChange(of: reduceMotion) { enabled in
                if enabled {
                    applyTarget(items.isEmpty ? nil : items, reduceMotion: true)
                }
            }
    ...
    private var visibleContent: some View {
        if presentation.phase == .empty {
            IndexResultCardEmptyState(emptyText: emptyText)
        } else if let displayedItems = presentation.snapshot {
            ScrollView {
                IndexResultCardStack(
                    items: displayedItems,
                    revealsItems: animatesItemUpdates,
                    valueMotionLimit: animatesItemUpdates ? 8 : 0
                )
                .padding(8)
            }
            .opacity(resultOpacity)
            .allowsHitTesting(resultInteractionEnabled)
            .accessibilityHidden(!resultInteractionEnabled)
        }
    }

    @MainActor
    private func applyTarget(_ target: [IndexResultCardItem]?, reduceMotion: Bool) {
        ...
        default:
            withTransaction(ToolMotion.disabledTransaction) {
                presentation = next
                completionRequest = nil
                animatesItemUpdates = false

                switch action {
                case let .appear(_, fadesIn) where fadesIn:
                    resultOpacity = 0
                case .settle:
                    resultOpacity = target == nil ? 0 : 1
                case .appear, .exit, .none, .update:
                    break
                }
            }
        }
        ...
        case let .appear(generation, _):
            startAppearance(generation: generation)
        case let .exit(generation):
            startExit(generation: generation)
    ...
    @MainActor
    private func startAppearance(generation: Int) {
    ...
    @MainActor
    private func startExit(generation: Int) {
