...
/// Values are stored as packed sRGB bytes so the parser has one auditable
/// source for aliases such as gray/grey.
public enum CSSNamedColors {
    public static var count: Int { packed.count }

    private static let packed: [String: UInt32] = [
    ...
    public static func color(named name: String) -> CSSColor? {
        guard let value = packed[name.lowercased()] else { return nil }
        let red = Double((value >> 16) & 255) / 255
        let green = Double((value >> 8) & 255) / 255
        let blue = Double(value & 255) / 255
        return try? CSSColor(components: SIMD3(red, green, blue), space: .sRGB, alpha: 1)
    }

    static func hasName(withPrefix prefix: String) -> Bool {
        packed.keys.contains { $0.hasPrefix(prefix) } || "transparent".hasPrefix(prefix)
    }
}
