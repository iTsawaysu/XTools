...
struct MotionSourceContractTests {
    @Test func toolMotionOwnsShellToastAndDiagnosticAnimations() throws {
        ...
        contains(root, ".toolTransition(\n                    ToolMotion.Transition.commandPalette,\n                    reduceMotion: reduceMotion\n                )", "v3: the first palette mount must keep the dedicated scale-and-settle transition")
        ...
        contains(root, ".allowsHitTesting(presentation.shows)", "The retained palette scrim must stop hit testing immediately on close")
        ...
        contains(commandPalette, ".animation(\n            ToolMotion.animation(ToolMotion.Preset.modal, reduceMotion: reduceMotion),\n            value: isPresentationReady\n        )", "The retained panel must animate visibility with the shared modal recipe")
        ...
        contains(commandPalette, "private struct CommandPaletteVisibilityModifier: @MainActor AnimatableModifier", "Retained palette sessions must animate from presentation-owned progress without lifecycle timers")
        contains(commandPalette, "y: reduceMotion || !isPresented", "Palette exit must preserve the original opacity-and-scale transition without adding upward motion")
        ...
        contains(root, "if viewModel.selectedToolID != toolID,\n           !reduceMotion,", "v3: palette launches must skip same-tool and Reduce Motion continuity flights")
