...
struct MotionSourceContractTests {
    @Test func toolMotionOwnsShellToastAndDiagnosticAnimations() throws {
        ...
        contains(motion, "static let pageArrival = Curve.smoothOut(duration: Duration.arrival)", "v3: tool-switch arrival must be the single whitelisted page transition preset")
        contains(host, ".toolPageArrival(id: pageArrivalID)", "v3: tool detail host must replay page switches through the shared host-owned arrival modifier")
        contains(root, ".toolAnimation(ToolMotion.Preset.pageArrival, value: viewModel.selectedToolID)", "v3: the detail column must key the arrival transaction to the selection so every launch path animates")
