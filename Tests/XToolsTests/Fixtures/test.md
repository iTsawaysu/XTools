...
预期：共同前缀裁剪后只计算最后一个元素的差异，不应因完整 3500×3500 行数提前报“对比内容过大”；实际 middle 超过预算时仍需稳定拒绝。
...
预期：共同前缀裁剪后只计算最后一行差异，4000 行稀疏变化也不得因完整矩阵预算提前失败。
...
生成左右各 4000 行且每一行都不同，使裁剪后的实际 middle 仍为 4000×4000，从而触发 LCS 预算。
