...
struct Base64SecuritySourceContractTests {
    ...
    @Test func securityTransformPagesUseSharedWorkbenchWithIntentionalExecutionPolicies() throws {
        ...
        contains(textEncryption, ".onChange(of: input) { _ in clearResult() }", "Text encryption input edits must clear stale security output until the next explicit action")
        contains(textEncryption, ".onChange(of: password) { _ in clearResult() }", "Text encryption key edits must clear stale security output until the next explicit action")
