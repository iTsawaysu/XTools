...
struct EditableDiffSourceContractTests {
    ...
    @Test func editableDiffWorkspaceUsesSingleOuterScrollAndWrapping() throws {
        ...
        contains(source, "static let paneGap: CGFloat = 8", "Diff panes must use the fixed editor-bay gap")
        ...
        contains(source, "static let paneCornerRadius: CGFloat = ToolMetrics.CornerRadius.field", "Independent diff panes must own the standard field radius")
        ...
        contains(source, "layer?.backgroundColor = NSColor.clear.cgColor", "The outer diff stage must be clear")
        ...
        contains(source, "layer?.borderColor = NSColor(ToolTheme.border).cgColor\n        layer?.borderWidth = 0.5", "Individual diff panes must use the standard 0.5pt border to match other 50/50 tools")
        ...
        contains(source, "NSColor.clear.setFill()", "The fixed split gap must be clear")
