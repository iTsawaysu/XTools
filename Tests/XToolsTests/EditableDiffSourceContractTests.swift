...
struct EditableDiffSourceContractTests {
    ...
    @Test func editableDiffWorkspaceUsesSingleOuterScrollAndWrapping() throws {
        ...
        contains(source, "IndexPanel(\"对比\")", "Diff diagnostics must stay inside the unified Clay Panel chrome")
        contains(source, ".indexWorkspaceDiagnostic(diagnosticText, tone: diagnosticTone)", "Diff chrome must reuse the shared panel diagnostic preference consumer")
        doesNotContain(source, ".indexWorkspaceDiagnostic(error ?? warning", "Diff diagnostics must not rely on the removed page-level preference consumer")
        ...
        contains(source, ".terminal(\"DIFF\")", "Diff workspace must show one compact workbench tag instead of pane-local Original/Compared labels")
        ...
        contains(source, "static let textInset = NSSize(width: 34, height: 14)", "Diff text must start close to the compact ruler and panel top")
        contains(source, "static let rulerWidth: CGFloat = 30", "Diff line numbers must use the compact editor gutter")
        contains(source, "static let lineNumberLeadingPadding: CGFloat = 3", "Compact line numbers must keep a small leading inset")
        contains(source, "static let lineNumberTrailingPadding: CGFloat = 3", "Compact line numbers must keep enough trailing separation")
        contains(source, ".monospacedSystemFont(ofSize: 10, weight: .regular)", "A 30-point ruler must fit four-digit line numbers without clipping")
        contains(source, "static let paneGap: CGFloat = 8", "Diff panes must use the tighter fixed editor-bay gap")
        contains(source, "static let dividerThickness: CGFloat = paneGap", "The fixed split divider must provide the visual gap between independent panes")
        contains(source, "static let stageInset: CGFloat = 8", "The diff editor bay must use a tighter shared stage inset")
        contains(source, "static let frameCornerRadius: CGFloat = 0", "The diff editor bay must not read as another nested rounded frame")
        contains(source, "static let frameBorderWidth: CGFloat = 0", "The diff editor bay must not draw a second outer editor border")
        contains(source, "static let paneCornerRadius: CGFloat = 7", "Independent diff panes must own the compact editor-surface radius")
        doesNotContain(source, "paneBorderWidth", "Diff panes must not retain the full-perimeter border that creates a double center rail")
        ...
        contains(source, "layer?.backgroundColor = IndexDiffNSPalette.panelBackground(for: effectiveAppearance).cgColor", "The outer diff stage must expose the panel material around independent editor panes")
        contains(source, "layer?.borderColor = NSColor.clear.cgColor", "The outer diff host must avoid a second nested editor border")
        ...
        contains(source, "layer?.backgroundColor = IndexDiffNSPalette.editorBackground(for: effectiveAppearance).cgColor", "Individual diff panes must use the shared editor background token")
        contains(source, "layer?.borderColor = NSColor.clear.cgColor\n        layer?.borderWidth = 0", "Individual diff panes must explicitly suppress the full border that produced the double center rail")
        ...
        contains(source, "IndexDiffNSPalette.panelBackground(for: effectiveAppearance).setFill()", "The fixed split gap must read as quiet panel spacing rather than an editor divider")
        ...
        doesNotContain(source, "NSBezierPath(rect: NSRect(x: bounds.width - 0.5, y: 0, width: 0.5, height: bounds.height)).fill()", "Diff line-number gutters must not draw a hard divider between line numbers and editor text")
